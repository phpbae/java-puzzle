...
public class Main {
    ...
    public static void main(String[] args) {
		 ...
		 * MSB 0 = 양수 MSB 1 = 음수
		 * 혼합 자료형 연산 : long + int 이기 때문에, int -> long 으로 확장변환(묵시적 형변환)이 일어남 이때, 부호확장도 함께 일어남.
		...
		/**
		 * 변수 교환
		 * 하나의 표현식에 동일한 변수를 여러 번 할당하지 말아라!(특별한 기교도 부리지마)
		 * 자바는 피연산자를 왼쪽에서 오른쪽으로 연산 /  x ^= expr 형태의 표현식을 계산 시, x는 expr이 계산되기 전에 추출되고 expr이 계산된 이후에 합쳐진다.
		 * 밑에 프로그램에서 x가 2번 추출되지만, 할당이 일어나기전에 추출된다.
		 * 대부분의 컴파일러는 x ^= expr에서 expr을 먼저 계산하고 x의 값을 추출한다.
		 */
